...
void TCPSender::send_segment(TCPSegment seg, uint64_t absolute_index, bool resend) {
    seg.header().seqno = wrap(absolute_index, _isn);

    if (resend) {
        _outstanding_segments.push_front(seg);
    } else {
        _outstanding_segments.push_back(seg);
    }
    _segments_out.push(seg);

    cout << "[send_segment] sent(seqno: " << seg.header().seqno << ", "
         << "syn: " << seg.header().syn << ", fin: " << seg.header().fin << "): [" << absolute_index << ", "
         << absolute_index + seg.length_in_sequence_space() << ")" << endl;

    _syn_sent |= seg.header().syn;
    _fin_sent |= seg.header().fin;

    _bytes_in_flight += seg.length_in_sequence_space();
    _next_absolute_index = max(_next_absolute_index, absolute_index + seg.length_in_sequence_space());
    ...
    if (seg.length_in_sequence_space() != 0 && !_timer.is_running()) {
        _timer.start();
    }
}

void TCPSender::send_segment(TCPSegment seg) { send_segment(seg, _next_absolute_index); }
...
void TCPSender::fill_window() {
    ...
    while (_next_absolute_index < first_unacceptable_absolute_index && !_fin_sent) {
        ...
        cout << "[fill_window] size: " << size << ", str.size: " << str.size() << ", payload: " << str << endl;
...
void TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
    ...
    if (ack_absolute_index < prev_ack_absolute_index) {
        return;
    }
    _ackno = ackno;
